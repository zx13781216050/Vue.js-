		...
		switch(currentState){
			...
			case State.text:
				if(isAlpha(char)){
					...
					//1.遇到字符<，切换到标签开始状态
					currentState = State.tagOpen
					//2.从文本状态-->标签开始状态,此时应该创建文本Token，并添加到tokens数组中
			...
			case State.tagEndName:
				if(isAlpha(char)){
					//1.遇到字母，不需要切换状态，当需要将当前字符缓存到chars数组
					chars.push(char)
					//2.消费当前字符
					str = str.slice(1)
				}else if(char === '>'){
					//1.遇到字符>，切换到初始状态
					currentState = State.initial
					//2.从结束标签名称状态-->初始状态，应该保存技术标签名称Token
					//注意，此时chars数组中缓存的字符就是标签名称
					tokens.push({
						type:'tagEnd',
						name:chars.join('')
